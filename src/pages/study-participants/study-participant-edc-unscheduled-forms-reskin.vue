<template>
  <div class="study-particip-details-block pt-10 pl-0" :class="{'telehealth-call': telehealthCallStatus}">
    ...
    <b-row class="mt-4" id="edcFormDetail">
      <b-col cols="12" class="p-0">
        <ParticipantInfo :areActionsAllowed="areActionsAllowed" :participantDetails="participantInfo" />
      </b-col>
      <b-col cols="12" class="pl-0 pr-0" v-if="unscheduledForms.length > 0">
        <b-row class="mb-20">
          <div class="primaryBackGround">
            <div v-for="(unscheduleForm, index) in unscheduledForms" :key="index" :class="{'formBorder': index!=unscheduledForms.length - 1}">
              ...
              <FormPreview :formData="unscheduleForm.form.fields" :formSubmitted="formSubmitted" />
            </div>
            <div class="pt-40 pl-30">
              <b-btn size="sm" class="fs-12 mr-5" variant="outline-secondary" @click="cancelForm">Cancel, do not save</b-btn>
              <b-btn size="sm" class="fs-12 mr-5" variant="outline-primary" @click="validateDataAndSave('DRAFT')" :disabled="canCraAccess || canDmAccess || canSiteDataLockAccess">Save as draft</b-btn>
              <b-btn size="sm" class="fs-12 primaryBgColor" variant="primary" @click="validateDataAndSave('SUBMITTED')" :disabled="canCraAccess || canDmAccess || canSiteDataLockAccess">Save and submit</b-btn>
            </div>
          </div>
        </b-row>
        <b-row>
  ...
  data() {
    return {
      ...
      navigationOptions: [
        ...
        {name: "Packet Details", isLink: false, route: null, params: null, displayOrder: 3}
      ],
      packetName: null,
      areActionsAllowed: {
        document: false,
