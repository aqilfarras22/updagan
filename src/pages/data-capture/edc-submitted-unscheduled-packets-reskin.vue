<template>
  <div class="pt-10 mb-5 component" :class="{'telehealth-call': telehealthCallStatus}">
    ...
    <PiiAndNonPiiInfo class="clear-both" v-if="participantInfo" :participantDetails="participantInfo"></PiiAndNonPiiInfo>
    <b-row>
      <b-col cols="12" class="p-0">
        <ParticipantInfo class="mt-10" @verificationConfirmed="verificationConfirmed" @approvalConfirmed="approvalConfirmed"  v-if="participantInfo && generalInfo" :edcFormInfo="generalInfo" :participantDetails="participantInfo" :areActionsAllowed="{document: false, telehealth: true, edcforms: false, approval: true, verifyBtn: true}"/>
      </b-col>
    </b-row>
    <b-row>
      <b-col cols="12" class="p-0">
          ...
          <b-row v-else no-gutters>
              ...
              <b-tabs v-model="tabIndex">
                <b-tab title="Latest Version"  @click="openTab(1)">
                  ...
                  <b-card-group v-if="!edit" v-for="(unscheduleAnswers, index) in answersObj" :key="index" :class="{'border-top': index > 0, 'pb-15' : index+1 === answersObj.length, 'pb-5' : index+1 !== answersObj.length}">
    ...
    verificationConfirmed(status) {
      if (this.generalInfo.verificationStatus !== status) {
        ...
        this.$api.updateEdcUnscheduleFormResponseVersionsResource(
          ...
          this.latestVersionData.unscheduledPacketResponseVersionId,
    ...
    getEDCUnscheduledPacketForms() {
      this.$api
        ...
        .then(response => {
          ...
          this.alreadySavedUnscheduleForms = response.body;
          this.latestVersionData = response.body;
          // get the form history versions
          this.getEDCFormHistoryList();
   ...
   openTab(tabNumber) {
    ...
    toggleParticipant(participant) {
      let params = {
          ...
          unscheduledPacketResponseId: this.unscheduledPacketResponseId
        };
