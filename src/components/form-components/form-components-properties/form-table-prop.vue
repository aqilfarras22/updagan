<template>
  <b-row>
    <h6 class="pl-12 pt-40 fs-20 title-color">
      <b>Table options</b>
    </h6>
    <b-row class="pt-20 mb-20">
      <b-col cols="12" class="fw-bold fs-13 mb-10" align-self="center">
        Field Requirement.
      </b-col>
      <b-col align-self="center">
        <b-form-checkbox @change="formTableData.required = !formTableData.required" v-model="formTableData.required" class="pl-0">
    ...
    <b-row class="pb-12">
      ...
      <b-col cols="12" class="fw-bold mt-30">
        Field Name
      </b-col>
      ...
      <b-col cols="12 pt-12">
        <b-form-input type="text" v-model="formTableData.fieldName" placeholder="Enter field name" class="field-h40 form-control fs-15" size="sm" v-bind:class="{'is-danger': formTableData.fieldNameErrorMsg }" v-on:keyup.native="checkForErrors" autocomplete="off"></b-form-input>
        <span class="error-style" v-if="formTableData.fieldNameErrorMsg">{{formTableData.fieldNameErrorMsg}}</span>
      </b-col>
    </b-row>
  </b-row>
</template>
...
export default {
  ...
  props: ['formTableSchema'],
  mounted() {
    this.formTableData = this.formTableSchema;
    this.notifications = this.formTableData.notifications;
  },
  computed: {},
  methods: {
    addNewNotification() {
      const notificationSchema = _.cloneDeep(this.notificationSchema);
      notificationSchema.name = `${notificationSchema.name} ${this.formTableData.notifications
        .length + 1}`;
    ...
    saveNotification(selectedChoice) {
      ...
      this.formTableData.notifications = this.notifications;
    },

    removeNotification(index, selectedChoice) {
      window.console.log(selectedChoice);
      // this.choicesToShow.push(selectedChoice);
      this.formTableData.notifications.splice(index, 1);
    },
    ...
    checkForErrors() {
      if (this.formTableData.fieldName && this.formTableData.fieldName.trimEnd().length > 0) {
        this.formTableData.fieldNameErrorMsg = '';
      }
    },
  },
};
