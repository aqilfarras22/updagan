  ...
  <b-row class="fs-12">
    ...
    <b-row class="pt-18 mb-20">
      ...
      <b-col align-self="center" class="fs-15">
        <b-form-checkbox @change="scaleData.required = !scaleData.required" v-model="scaleData.required" class="pl-0">
    ...
    <b-row class="pb-12">
      ...
      <b-col cols="12" class="fw-bold fs-13 mt-20">
        Field Name
      </b-col>
      ...
      <b-col cols="12 pt-12">
        <b-form-input type="text" v-model="scaleData.fieldName" placeholder="Enter field name" class="field-h40 form-control fs-13" size="sm" v-bind:class="{'is-danger': scaleData.fieldNameErrorMsg }" v-on:keyup.native="checkForErrors" autocomplete="off"></b-form-input>
        <span class="error-style" v-if="scaleData.fieldNameErrorMsg">{{scaleData.fieldNameErrorMsg}}</span>
      </b-col>
    </b-row>
  </b-row>
</template>
  ...
  data() {
    return {
      ...
      choicesToShow: [
        ...
        'If answer violates a set threshold',
        'If answer is below/above a set weight',
      ],
    };
  },
  props: ['scaleSchema'],
  mounted() {
    this.scaleData = this.scaleSchema;
    this.notifications = this.scaleData.notifications;
  },
  computed: {},
  methods: {
    addNewNotification() {
      const notificationSchema = _.cloneDeep(this.notificationSchema);
      notificationSchema.name = `${notificationSchema.name} ${this.scaleData.notifications.length
        + 1}`;
    ...
    saveNotification(selectedChoice) {
      ...
      this.scaleData.notifications = this.notifications;
    },

    removeNotification(index, selectedChoice) {
      window.console.log(selectedChoice);
      // this.choicesToShow.push(selectedChoice);
      this.scaleData.notifications.splice(index, 1);
    },
    ...
    checkForErrors() {
      if (this.scaleData.fieldName && this.scaleData.fieldName.trimEnd().length > 0) {
        this.scaleData.fieldNameErrorMsg = '';
      }
    },
  },
};
