<template>
  <b-row class="fs-12">
    <h6 class="pl-12 pt-20 fs-20 title-color">
      <b>Date and Time options</b>
    </h6>
    <b-row class="pt-18 mb-20">
      <b-col cols="12" class="fw-bold fs-13" align-self="center">
        Field Requirement.
      </b-col>
      <b-col align-self="center" class="fs-15">
        <b-form-checkbox @change="dateTimeData.required = !dateTimeData.required" v-model="dateTimeData.required" class="pl-0">
      ...
      <b-col cols="12">
        <div v-for="(notification, index) in notifications" :key="index" v-if="notifications && notifications.length" class="pt-10">
          <b-col class="fs-11 notificationBorder" v-if="!notification.mode">
            <b-form-select v-model="notification.triggerCondition.selectedOptionIndex" :options="choicesToShow" class="mb-5 fs-12" size="sm">
            </b-form-select>
    ...
    <b-row class="pb-12">
      ...
      <b-col cols="12" class="fw-bold fs-13 mt-30">
        Field Name
      </b-col>
      ...
      <b-col cols="12 pt-12">
        <b-form-input type="text" v-model="dateTimeData.fieldName" placeholder="Enter field name" class="field-h40 form-control fs-15" size="sm" v-bind:class="{'is-danger': dateTimeData.fieldNameErrorMsg }" v-on:keyup.native="checkForErrors" autocomplete="off"></b-form-input>
        <span class="error-style" v-if="dateTimeData.fieldNameErrorMsg">{{dateTimeData.fieldNameErrorMsg}}</span>
      </b-col>
    </b-row>
  </b-row>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      notifications: [],
      notificationSchema: find(schemas, 'fieldType', 'DateTime').schemas.notificationSchema,
    };
  },
  props: ['dateTimeSchema'],
  mounted() {
    this.dateTimeData = this.dateTimeSchema;
    this.notifications = this.dateTimeData.notifications || [];
  },
  computed: {
    choicesToShow() {
      this.choicesGiven = [
  ...
  methods: {
    ...
    saveNotification(index) {
      ...
      if (!this.notifications[index].triggerCondition.selectedOptionVal) {
        ...
        this.dateTimeData.notifications = this.notifications;
        this.dateTimeData.notifications[index].name = `${
          this.dateTimeData.notifications[index].name
        } ${index + 1}`;
      }
    },
    ...
    removeNotification(index) {
      ...
      this.dateTimeData.notifications.splice(index, 1);
      const tempNotifications = this.dateTimeData.notifications;
      this.dateTimeData.notifications = [];
      tempNotifications.forEach((notification, ind) => {
        const tempNotification = notification;
        tempNotification.name = `Notification ${ind + 1}`;
        this.dateTimeData.notifications.push(tempNotification);
      });
    },

    addContact(index) {
      this.dateTimeData.notifications[index].contacts.push({
        status: true,
        name: 'Sari Patel(PhV)',
        email: false,
        sms: false,
    ...
    findOptionVal(index, optIndex) {
      ...
      if (!this.dateTimeData.choices[findIndex(this.dateTimeData.choices, ['index', optIndex])]) {
        this.dateTimeData.notifications[index].triggerCondition.selectedOptionVal = '';
      } else {
        this.dateTimeData.notifications[
          index
        ].triggerCondition.selectedOptionVal = this.dateTimeData.choices[
          findIndex(this.dateTimeData.choices, ['index', optIndex])
        ].value;
      }
    },

    checkForErrors() {
      if (this.dateTimeData.fieldName && this.dateTimeData.fieldName.trimEnd().length > 0) {
        this.dateTimeData.fieldNameErrorMsg = '';
      }
    },
  },
};
