<template>
  <b-row class="fs-12">
    <h6 class="pl-12 pt-20 fs-20 title-color">
      <b>TextArea options</b>
    </h6>
    <b-row class="pt-18 mb-20">
      <b-col cols="12" class="fw-bold mb-10" align-self="center">
        Field Requirement.
      </b-col>
      <b-col align-self="center">
        <b-form-checkbox @change="formTextAreaData.required = !formTextAreaData.required" v-model="formTextAreaData.required" class="pl-0">
          ...
          <b-col class="fs-11 notificationBorder" v-if="!notification.mode">
            ...
            <b-row class="pb-10 lh-3" v-if="notification.thresholdFalg">
              ...
              <b-col cols="12" class="p-0">
                ...
                <b-col cols="12" class="p-0">
                  <b-form-input type="number" class="fs-12" min="0" v-model="notification.threshold.minValue" v-if="notification.threshold.minLimitFlag" size="sm" v-validate="{regex: /^\d+$/, required: true}" name="numberMin" v-bind:class="{'input': true, 'is-danger': errors.has('numberMin')}" autocomplete="off">
                  </b-form-input>
                </b-col>
              </b-col>
              <b-col cols="12" class="p-0">
            ...
            <b-row class="pb-10 lh-3">
              ...
              <b-col cols="7" class="p-0">
                <b-btn class="btn br-1 pull-right notifyButton" variant="outline-primary" disabled>Select people to notify</b-btn>
              </b-col>
            </b-row>
            <b-row>
    ...
    <b-row class="pb-12">
      ...
      <b-col cols="12" class="fw-bold mt-20 fs-12">
        Field Name
      </b-col>
      ...
      <b-col cols="12 pt-12">
        <b-form-input type="text" v-model="formTextAreaData.fieldName" placeholder="Enter field name" class="field-h40 form-control fs-15" size="sm" v-bind:class="{'is-danger': formTextAreaData.fieldNameErrorMsg }" v-on:keyup.native="checkForErrors" autocomplete="off"></b-form-input>
        <span class="error-style" v-if="formTextAreaData.fieldNameErrorMsg">{{formTextAreaData.fieldNameErrorMsg}}</span>
      </b-col>
    </b-row>
  </b-row>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      choicesToShow: ['If an answer is provided', 'If an answer is not provided'],
    };
  },
  props: ['formTextAreaSchema'],
  mounted() {
    this.formTextAreaData = this.formTextAreaSchema;
    this.notifications = this.formTextAreaData.notifications;
  },
  computed: {},
  methods: {
    addNewNotification() {
      const notificationSchema = _.cloneDeep(this.notificationSchema);
      notificationSchema.name = `${notificationSchema.name} ${this.formTextAreaData.notifications
        .length + 1}`;
    ...
    saveNotification(selectedChoice) {
      ...
      this.formTextAreaData.notifications = this.notifications;
    },

    removeNotification(index, selectedChoice) {
      window.console.log(selectedChoice);
      // this.choicesToShow.push(selectedChoice);
      this.formTextAreaData.notifications.splice(index, 1);
    },
    ...
    checkForErrors() {
      if (this.formTextAreaData.fieldName && this.formTextAreaData.fieldName.trimEnd().length > 0) {
        this.formTextAreaData.fieldNameErrorMsg = '';
      }
    },
  },
};
