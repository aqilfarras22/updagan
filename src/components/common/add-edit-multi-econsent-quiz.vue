  ...
  <div class="add-edit-quiz-card">
    ...
    <div class="row pt-20">
      <div class="col-lg-9 col-md-9 col-sm-6 col-xs-2 p-0">
          <img src="/static/img/econsent/delete.svg" v-on:click="deleteQuestion()" class="delete-btn">
      </div>
      <div class="col-lg-3 col-md-3 col-sm-6 col-xs-10 text-right p-0">
        <button class="cancel-btn" v-on:click="cancelQuestion()">Cancel</button>
        <button class="save-btn" v-on:click="saveQuestion()">Save</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    this.saveInitialSingleChoiceAnswers(this.options);
    this.questionTypeChanged(this.addEditQuestion);
  },
  methods: {
    saveInitialSingleChoiceAnswers(options) {
      if (options.type === 'singleChoice' && options.opType === 'toEdit') {
        this.firstAnswer = options.options[0].body;
        this.secondAnswer = options.options[1].body;
    ...
    questionTypeChanged(addEditQuestion) {
      ...
      if (this.addEditQuestion.options
        ...
        && isBooleanType) {
        this.addEditQuestion.options[0].body = isBooleanType ? 'Yes' : this.addEditQuestion.options[0].body || '';
        this.addEditQuestion.options[1].body = isBooleanType ? 'No' : this.addEditQuestion.options[1].body || '';
      } else if (addEditQuestion.type === 'singleChoice') {
        this.addEditQuestion = this.addEditQuestion;
        if (addEditQuestion.opType === 'toAdd') {
          this.addEditQuestion.options = [{ body: '', sequence: 1 }, { body: '', sequence: 2 }];
        } else if (addEditQuestion.opType === 'toEdit') {
          this.addEditQuestion.options[0].body = this.firstAnswer;
          this.addEditQuestion.options[1].body = this.secondAnswer;
          this.addEditQuestion.expectedAnswer = this.expectedAnswer;
        }
      } else {
        this.addEditQuestion.options = [{ body: '', sequence: 1 }, { body: '', sequence: 2 }];
      }
    ...
    saveQuestion() {
      ...
      if (validQuestion) {
        ...
        toastr.error('Please fill all the fields in the question');
      }
    },
    cancelQuestion() {
      swal({
