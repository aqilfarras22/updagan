          ...
          <form class="pt-20 otp_form">
              ...
              <div>
                <div class="form-group">
                  <div class="link-style"> <a v-on:click="incorrectNumber" class="cursor_pointer">Incorrect mobile number?</a></div>
                </div>
              </div>
              <div class="form-group pt-20">
                ...
                <div v-if="errors.any()">
                  <button class="btn btn-lg btn-block btn-disabled br-25" :disabled="!correctOTP">Continue</button>
                </div>
                <div v-if="!errors.any()">
...
export default {
  ...
  computed: {
    clientVal() {
      return this.$store.getters.appStateClientId;
    },
  },
  mounted() {
  ...
  methods: {
    ...
    incorrectNumber() {
      if (window.localStorage.getItem('thread-token')) {
        this.decoded = jwtDecode(window.localStorage.getItem('thread-token'));
        this.$router.push({ name: 'aboutYou', params: { userId: this.decoded.id } });
      }
    },
    confirmOTP() {
      ...
      this.$api.verifyOtpFact({ token: this.token, otp: this.otp }).then(
        response => {
          ...
          if (window.localStorage.getItem('thread-token')) {
            this.decoded = jwtDecode(window.localStorage.getItem('thread-token'));
            if (this.decoded.userRole === 'SystemAdmin') {
