...
export default {
  ...
  computed: {
    title() {
      if (this.verified) {
        return 'Have you thoroughly unverified this source document?';
      }
  ...
  methods: {
    ...
    yesAction() {
      const onSuccess = () => {
        this.$toastr.success(`${this.verified ? 'Unverification' : 'Verification'} confirmed`);
        this.$emit('confirmed', { verified: !this.verified });
        this.hide();
      };

      if (this.callApi) {
        const patchId = !this.verified ? 'verify' : 'unVerify';

        this.$api
          .setQueryToolStudyFormStatus({ fId: this.formId, patchId })
          .then(onSuccess)
          .catch(error => {
