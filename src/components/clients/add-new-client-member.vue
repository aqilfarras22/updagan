      ...
      <div class="container">
        <div class="row has-header">
          <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 pt-20">
            <span class="color_ff fs-28"> New {{clientObj.name}} Contact</span>
          </div>
        </div>
        <div class="row p-15">
          <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 card-container">
            <div class="team-card">
              <div class="row" v-for="(clientMember,index) in clientMembers">
                <div class="col-md-4 col-sm-4 card-height">
                  <p class="inner-card">Name</p>
                  <p><span class="inner-card-name">First Name:</span>
                    <input type="text" v-validate="'required'" name="firstName" v-model="clientMember.firstName" class=" first-name-value" :class="{'input': true, 'is-danger': errors.has('firstName'), 'is-success': !errors.has('firstName') }"
                  ...
                  <p><span class="inner-card-name">Last Name:</span>
                    <input type="text" v-validate="'required'" name="lastName" v-model="clientMember.lastName" class=" first-name-value" :class="{'input': true, 'is-danger': errors.has('lastName'), 'is-success': !errors.has('lastName') }"
                ...
                <div class="col-md-12 col-sm-12 pb-20">
                  <span v-if="clientMembers.length>1" class="cursor_pointer remove-color" v-on:click="removeContact(index)">Remove</span>
                </div>
              </div>
              <div class="row">
                <div class="pt-20">
                  ...
                  <div class="col-md-8 text-right">
                    <button type="button" class="btn btn-md cancel-btn" v-on:click="goToClientList"> Cancel</button>
                    <button type="button" class="btn btn-md save-btn" v-on:click="saveUserInfo(clientMembers)"> Invite </button>
                  </div>
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    addAnotherContact() {
      this.clientMembers.push({ firstName: '', lastName: '', username: '', contactType: '' });
    },
    removeContact(index) {
    ...
    saveUserInfo() {
      this.$api.addNewClientMemberInfoFact(this.clientId, this.clientMembers).then(
        () => {
          toastr.success('client member succesfully');
